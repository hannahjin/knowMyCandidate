...
Parse.Cloud.job("pull_news_test", function(request, response) {
	// Retrieve all the candidates
	var Candidate = Parse.Object.extend("Candidate");
	var query = new Parse.Query(Candidate);
	  ...
	  success: function(results) {
		...
		for (var i = 0; i < 1; i++) {
			...
			if (candidateNames.indexOf(candidateFirstAndLast) == -1) {
				...
				// TEST GET ONLY 1 URL
				// Bing Search API URL to get the top 5 news results for a query
				var formatted_url = 'https://api.datamarket.azure.com/Bing/Search/v1/News?Query=%27'+object.get('firstName')+ '%20' +object.get('lastName') +'%27&$top=1&$format=json';
					...
					success: function(httpResponse) {
						...
						for (var article in result_list) {
							...
							(function(article){
								checkDuplicateQuery.find({
								  success: function(duplicateResults) {
								  	// TEST: always save, even if duplicate
									//if (duplicateResults.length == 0) {
										var NewsFeed = Parse.Object.extend("Newsfeed");
										var newsFeed = new NewsFeed();
										...
										newsFeed.set("source", result_list[article].Source);
										newsFeed.set("date", new Date(result_list[article].Date));
										console.log("attempting now for candidate: " + candidate + " and url: " + result_list[article].Url);
										thumbnail.setThumbnailandSaveNewsfeed(newsFeed, result_list[article].Url, candidate); 							    	
									//}
								  },
								  error: function(error) {
